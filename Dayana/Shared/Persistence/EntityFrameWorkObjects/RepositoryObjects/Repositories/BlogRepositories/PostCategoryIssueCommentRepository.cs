...
public class PostCategoryIssueCommentRepository : Repository<PostCategoryIssueComment>, IPostCategoryIssueCommentRepository
{
    ...
    public async Task<List<PostCategoryIssueComment>> GetPostCategoryIssueCommentsByFilterAsync(DefaultPaginationFilter filter)
    {
    ...
    public async Task<PostCategoryIssueComment> GetPostCategoryIssueCommentByIdAsync(int id)
    {
        var data = await _queryable
            .SingleOrDefaultAsync(x => x.Id == id);

        if (data == null)
            throw new NullReferenceException(GenericErrors<PostCategoryIssueComment>.NotFoundError("id").ToString());
